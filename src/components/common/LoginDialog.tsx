  ...
  const handleSubmit = async () => {
    ...
    try {
      if (mode === 'login') {
        ...
        const req: RegisterRequest = { phone, code, nickname };
        await register(req);
      }
      onClose();
    } catch (e) {
      setError(mode === 'login' ? '登录失败，请检查验证码' : '注册失败，请重试');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      ...
      >
        ...
        {/* 验证码 */}
        <div className="mb-3">
          <label className="block text-text-secondary text-xs mb-1">验证码</label>
          <div className="flex items-center gap-2">
            <input
              ...
              placeholder="请输入验证码"
              maxLength={6}
              className="flex-1 bg-bg-secondary border border-border text-text-primary text-sm px-3 py-2 rounded focus:outline-none focus:border-orange/50"
            />
            <button
              className={`px-3 py-2 rounded text-xs whitespace-nowrap transition-colors ${
                countdown > 0
                  ...
                  : 'bg-orange/15 border border-orange/30 text-orange hover:bg-orange hover:text-black'
              }`}
