...
import { formatPrice, formatChangePct, formatTime } from '../../utils/format';

export default function IndexBar() {
  const indices = useMarketStore((s) => s.indices);
  const indicesUpdatedAt = useMarketStore((s) => s.indicesUpdatedAt);
  ...
  return (
    <div className="flex gap-0 px-3 py-1 bg-[#0d0d0d] border-b border-border shrink-0 min-h-[50px]">
      {indices.length === 0 ? (
        <div className="flex items-center text-text-muted text-xs">加载指数数据...</div>
      ) : (
        <>
          {indices.map((idx) => {
          ...
          {indicesUpdatedAt && (
            <div className="ml-auto flex items-center pl-3 text-[10px] text-text-muted">
              更新 {formatTime(indicesUpdatedAt)}
            </div>
          )}
        </>
      )}
